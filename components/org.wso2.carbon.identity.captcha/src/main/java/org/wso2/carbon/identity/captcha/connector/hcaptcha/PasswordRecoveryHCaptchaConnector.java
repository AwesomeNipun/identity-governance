...
package org.wso2.carbon.identity.captcha.connector.hcaptcha;
 ...
 */
public class PasswordRecoveryHCaptchaConnector extends AbstractHCaptchaConnector {

    private static final Log log = LogFactory.getLog(PasswordRecoveryHCaptchaConnector.class);
    ...
    private static final String RECOVERY_QUESTION_PASSWORD_HCAPTCHA_ENABLE = "Recovery.Question.Password" +
            ".HCaptcha.Enable";
    private static final String RECOVERY_QUESTION_PASSWORD_HCAPTCHA_MAX_FAILED_ATTEMPTS = "Recovery.Question" +
            ".Password.HCaptcha.MaxFailedAttempts";

    private final String FORGOT_PASSWORD_HCAPTCHA_ENABLE = "Recovery.HCaptcha.Password.Enable";
    ...
    public CaptchaPreValidationResponse preValidate(ServletRequest servletRequest, ServletResponse servletResponse) throws CaptchaException {
        ...
        boolean forgotPasswordHcaptchaEnabled = CaptchaUtil.isHcaptchaEnabledForConnector(identityGovernanceService,
                servletRequest, FORGOT_PASSWORD_HCAPTCHA_ENABLE);
        String pathUrl = ((HttpServletRequest) servletRequest).getRequestURI();

        if ((CaptchaDataHolder.getInstance().isForcefullyEnabledHcaptchaForAllTenants() ||
                forgotPasswordHcaptchaEnabled) &&
        ...
        try {
            connectorConfigs = identityGovernanceService.getConfiguration(new String[]{
                            RECOVERY_QUESTION_PASSWORD_HCAPTCHA_ENABLE,
                            RECOVERY_QUESTION_PASSWORD_HCAPTCHA_MAX_FAILED_ATTEMPTS},
        ...
        for (Property connectorConfig : connectorConfigs) {
            if ((RECOVERY_QUESTION_PASSWORD_HCAPTCHA_ENABLE)
                    .equals(connectorConfig.getName())) {
                connectorEnabled = connectorConfig.getValue();
            } else if ((RECOVERY_QUESTION_PASSWORD_HCAPTCHA_MAX_FAILED_ATTEMPTS)
                    .equals(connectorConfig.getName())) {
        ...
        if (StringUtils.isBlank(maxAttemptsStr) || !NumberUtils.isNumber(maxAttemptsStr)) {
            log.warn("Invalid configuration found in the PasswordRecoveryHCaptchaConnector for the tenant - " +
                    user.getTenantDomain());
            return preValidationResponse;
        }
        ...
        if (currentFailedAttempts > maxFailedAttempts) {
            if (initializationFlow) {
                httpServletResponse.setHeader("hCaptcha", "true");
                httpServletResponse.setHeader("hCaptchaKey", CaptchaDataHolder.getInstance().getHCaptchaSiteKey());
                httpServletResponse.setHeader("hCaptchaAPI", CaptchaDataHolder.getInstance().getHCaptchaAPIUrl());
            } else {
                preValidationResponse.setCaptchaValidationRequired(true);
    ...
    public boolean verifyCaptcha(ServletRequest servletRequest, ServletResponse servletResponse)
        ...
        if (StringUtils.isBlank(hCaptchaResponse)) {
            throw new CaptchaClientException("hCaptcha response is not available in the request.");
        }

        return CaptchaUtil.isValidCaptcha(hCaptchaResponse);
    }
    ...
    private void addPostValidationData(ServletRequest servletRequest) {
        ...
        enabledSecurityMechanism.setMechanism("hCaptcha");
        Map<String, String> properties = new HashMap<>();
        properties.put("hCaptchaKey", CaptchaDataHolder.getInstance().getHCaptchaSiteKey());
        properties.put("hCaptchaAPI", CaptchaDataHolder.getInstance().getHCaptchaAPIUrl());
